...
<body>
				...
				<div class="layui-inline">
					...
					<div class="layui-input-inline">
						<select name="city" lay-verify="">
							...
							<option value="021">已入库</option>
							<option value="0571">未入库</option>
						</select>
					</div>
				</div>
	...
	<!-- 弹出层div(新增) -->
	<div id="mydivAdd"  style="display: none" method=>
		<form  class="layui-form" lay-filter="dataform" id="dataform" method="get">
			<input type="hidden" name="stoId">
			
			<div class="layui-form-item" >
				<label class="layui-form-label">货物类型：</label>
				<div class="layui-input-inline">
					<input type="text" name="typeGoods" required lay-verify="required"
			...
			<div class="layui-form-item" >
				...
				<div class="layui-input-inline">
					<input type="text" name="goodsNumber" required lay-verify="required"
						autocomplete="off" placeholder="货物数量" class="layui-input">
				</div>
			</div>
		...
		layui.use([ "element", "jquery", "layer", "form", "table" ],
				function() {
					...
					var tableIns = table.render({
						...
						,
						url : '../getAllStorAge.do'//'moSelectName.do?moName='+$("#moName").val() //数据接口
						,
						title : '出库数据表' //数据导出的标题
						,
						    ...
						    case 'delete':
						    	...
						    	if(checkStatus.data.length>0){
						    		for(var i=0;i<checkStatus.data.length;i++){
						    			if(i==0){
						    				id=checkStatus.data[i].stoId+id;
						    			}else{
						    				id=checkStatus.data[i].stoId+","+id;
						    			}
						    			
						    		}
						    		 $.post("../delStorAge.do","stoId="+id,function(back){
										//刷新数据表格
										tableIns.reload();
									 }); 
						    	}else{
				      ...
				      table.on('tool(userTable)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
				    	 ...
				    	 if(layEvent === 'del'){ //删除
				    	    layer.confirm('真的删除行么', function(index){
				    	   	 $.post("../delStorAge.do","stoId="+data.stoId,function(back){
									//刷新数据表格
									tableIns.reload();
								 });
				    	      layer.close(index);
